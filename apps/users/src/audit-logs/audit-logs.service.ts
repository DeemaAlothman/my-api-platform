...
export class AuditLogsService {
  ...
  async getLogs(
    ...
    if (allowedUserIds !== null) {
      ...
      const placeholders = allowedUserIds
        .map(() => `$${paramIdx++}`)
        .join(', ');
    ...
    const [countRows, rows] = await Promise.all([
      this.prisma.$queryRawUnsafe(countSql, ...params) as Promise<
        Array<{ total: string }>
      >,
      this.prisma.$queryRawUnsafe(dataSql, ...params) as Promise<
        Array<{
          id: string;
          userId: string | null;
