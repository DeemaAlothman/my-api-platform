...
export class HrReportsController {
  ...
  @ApiOperation({
    summary: 'توزيع الموظفين حسب القسم والجنس والجنسية ونوع العقد',
  })
  ...
  async turnover(
    ...
    const data = await this.service.turnover(
      year ? parseInt(year) : new Date().getFullYear(),
    );
    if (format === 'excel') {
      await sendExcel(
        res!,
        `hr-turnover-${data.year}`,
        ['الشهر', 'موظفون جدد', 'إنهاء خدمة'],
        data.hired.map((h) => {
          const t = data.terminated.find(
            (x) => String(x.month) === String(h.month),
          );
  ...
  async expiryDates(
    ...
    const data = await this.service.expiryDates(
      daysAhead ? parseInt(daysAhead) : 90,
    );
    if (format === 'excel') {
      await sendExcel(
        res!,
    ...
    if (format === 'excel') {
      await sendExcel(
        ...
        data.map((r: any) => [
          ...
          r.category ?? '',
          r.handedOverAt
            ? new Date(r.handedOverAt).toLocaleDateString('ar-SA')
            : '',
          r.status === 'WITH_EMPLOYEE' ? 'نشط' : 'مُرجع',
          r.returnedAt
            ? new Date(r.returnedAt).toLocaleDateString('ar-SA')
            : '',
          r.notes ?? '',
        ]),
      );
