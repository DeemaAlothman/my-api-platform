...
export class EmployeesController {
    ...
    await sendExcelMultiSheet(res, `employees-full-${today}`, [
      {
        ...
        headers: [
          ...
          'نتيجة التجربة', 'تاريخ إنهاء التجربة', 'اسم المستخدم',
        ],
        rows: mainRows,
      },
      {
  ...
  @Post('internal/find-by-user-id')
  findByUserIdInternal(
    @Headers('x-internal-token') token: string,
  ...
  resolveEmployeeIds(
    @Headers('x-internal-token') token: string,
    @Body() dto: { employeeIds: string[] },
  ) {
    const expected = process.env.INTERNAL_SERVICE_TOKEN;
    if (expected && token !== expected) {
      throw new UnauthorizedException('Invalid internal token');
    }
    return this.employees.resolveEmployeeIds(dto.employeeIds ?? []);
  }
