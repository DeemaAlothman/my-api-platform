  ...
  async deductionBreakdownReport(query: {
    ...
    const rows = payrolls.map((p) => {
      ...
      return {
        ...
        lateDeductionAmount: Number(bd.tardiness?.amount ?? bd.lateDeduction ?? 0),
        absenceDays: p.absentUnjustified || 0,
        absenceDeductionAmount: Number(bd.absence?.amount ?? bd.absenceDeduction ?? p.absenceDeductionAmount ?? 0),
        breakOverLimitMinutes: p.breakOverLimitMinutes || 0,
        breakDeductionAmount: Number(bd.breakOverLimit?.amount ?? bd.breakOverLimitDeduction ?? 0),
        totalDeduction: Number(bd.totalDeduction || p.deductionAmount || 0),
        proRationFactor: p.proRationFactor || 1,
      };
    });
