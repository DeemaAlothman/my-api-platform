...
export class DailyClosureService implements OnModuleInit {
  ...
  async processDayForAllEmployees(dateStr: string): Promise<{
    ...
    // C.4b: Create LATE alerts for display and justification (no salary impact)
    await this.createLateAlerts(dateStr);
    await this.createEarlyLeaveAlerts(dateStr);
    await this.createMissingClockInAlerts(dateStr);
  ...
  /**
   * C.4b: ينشئ تنبيه LATE لكل موظف سُجِّل تأخيره اليوم ولم يُنشأ له تنبيه بعد.
   * قراءة فقط من attendance_records — لا يعدّل الرواتب أو الخصومات.
   */
  private async createLateAlerts(dateStr: string): Promise<void> {
    try {
      const lateRecords = (await this.prisma.$queryRawUnsafe(
        `SELECT ar.id AS "recordId", ar."employeeId", ar."lateMinutes", e."userId"
         FROM attendance.attendance_records ar
         JOIN users.employees e ON e.id = ar."employeeId"
         WHERE ar.date = $1::date
           AND ar."lateMinutes" > 0
           ...
           AND NOT EXISTS (
             ...
             WHERE aa."employeeId" = ar."employeeId"
               AND aa.date = $1::date
               AND aa."alertType" = 'LATE'
           )`,
        dateStr,
      )) as Array<{ recordId: string; employeeId: string; lateMinutes: number; userId: string | null }>;

      for (const r of lateRecords) {
        try {
          await this.prisma.$queryRawUnsafe(
            `INSERT INTO attendance.attendance_alerts
               (id, "employeeId", date, "alertType", "attendanceRecordId", severity,
                message, "messageAr", status, "isAutoGenerated", "createdAt", "updatedAt")
             VALUES
               (gen_random_uuid(), $1, $2::date, 'LATE', $3,
                CASE WHEN $4::int >= 30 THEN 'HIGH' ELSE 'MEDIUM' END,
                $5, $6, 'OPEN', true, NOW(), NOW())`,
            r.employeeId, dateStr, r.recordId, r.lateMinutes,
            `Employee was late by ${r.lateMinutes} minutes on ${dateStr}`,
            `الموظف تأخر ${r.lateMinutes} دقيقة يوم ${dateStr}`,
          );
              ...
              `INSERT INTO users.notifications
               ...
               VALUES
                 (gen_random_uuid(), $1, 'ATTENDANCE_ALERT',
                  'تنبيه تأخر', 'Late Arrival Alert', $2, $3, false, $4::jsonb, NOW())`,
              r.userId,
              `تأخرت ${r.lateMinutes} دقيقة يوم ${dateStr} — يمكنك تقديم تبرير خلال 7 أيام`,
              `You were late ${r.lateMinutes} minutes on ${dateStr} — you can submit a justification within 7 days`,
              JSON.stringify({ alertType: 'LATE', attendanceRecordId: r.recordId, date: dateStr }),
            );
          }
        } catch (err) {
          this.logger.error(`createLateAlerts failed for ${r.employeeId} on ${dateStr}: ${(err as any)?.message}`);
        }
      }

      if (lateRecords.length > 0) {
        this.logger.log(`createLateAlerts: ${lateRecords.length} LATE alert(s) created for ${dateStr}`);
      }
    } catch (err) {
      this.logger.error(`createLateAlerts failed for ${dateStr}: ${(err as any)?.message}`);
    }
  }

  private async createEarlyLeaveAlerts(dateStr: string): Promise<void> {
    try {
      const earlyRecords = (await this.prisma.$queryRawUnsafe(
        `SELECT ar.id AS "recordId", ar."employeeId", ar."earlyLeaveMinutes", e."userId"
         FROM attendance.attendance_records ar
         JOIN users.employees e ON e.id = ar."employeeId"
         WHERE ar.date = $1::date
        ...
        dateStr,
      )) as Array<{ recordId: string; employeeId: string; earlyLeaveMinutes: number; userId: string | null }>;
          ...
          await this.prisma.$queryRawUnsafe(
            `INSERT INTO attendance.attendance_alerts
               (id, "employeeId", date, "alertType", "attendanceRecordId", severity,
                message, "messageAr", status, "isAutoGenerated", "createdAt", "updatedAt")
             VALUES
               (gen_random_uuid(), $1, $2::date, 'EARLY_LEAVE', $3,
                CASE WHEN $4::int >= 30 THEN 'HIGH' ELSE 'MEDIUM' END,
                $5, $6, 'OPEN', true, NOW(), NOW())`,
            r.employeeId, dateStr, r.recordId, r.earlyLeaveMinutes,
            `Employee left early by ${r.earlyLeaveMinutes} minutes on ${dateStr}`,
            `الموظف غادر مبكراً بمقدار ${r.earlyLeaveMinutes} دقيقة يوم ${dateStr}`,
          );
          ...
          if (r.userId) {
            await this.prisma.$queryRawUnsafe(
              `INSERT INTO users.notifications
               ...
               VALUES
                 (gen_random_uuid(), $1, 'ATTENDANCE_ALERT',
                  'تنبيه خروج مبكر', 'Early Leave Alert', $2, $3, false, $4::jsonb, NOW())`,
              r.userId,
              `غادرت مبكراً ${r.earlyLeaveMinutes} دقيقة يوم ${dateStr} — يمكنك تقديم تبرير خلال 7 أيام`,
              `You left ${r.earlyLeaveMinutes} minutes early on ${dateStr} — you can submit a justification within 7 days`,
              JSON.stringify({ alertType: 'EARLY_LEAVE', attendanceRecordId: r.recordId, date: dateStr }),
            );
          }
        } catch (err) {
          this.logger.error(`createEarlyLeaveAlerts failed for ${r.employeeId} on ${dateStr}: ${(err as any)?.message}`);
        }
      }

      if (earlyRecords.length > 0) {
        this.logger.log(`createEarlyLeaveAlerts: ${earlyRecords.length} EARLY_LEAVE alert(s) created for ${dateStr}`);
      }
    } catch (err) {
      this.logger.error(`createEarlyLeaveAlerts failed for ${dateStr}: ${(err as any)?.message}`);
    }
  }

  private async createMissingClockInAlerts(dateStr: string): Promise<void> {
    try {
      const records = (await this.prisma.$queryRawUnsafe(
        `SELECT ar.id AS "recordId", ar."employeeId", e."userId"
         FROM attendance.attendance_records ar
         JOIN users.employees e ON e.id = ar."employeeId"
         WHERE ar.date = $1::date
           ...
           AND NOT EXISTS (
             ...
             WHERE aa."employeeId" = ar."employeeId"
               AND aa.date = $1::date
               AND aa."alertType" = 'MISSING_CLOCK_IN'
           )`,
        dateStr,
      )) as Array<{ recordId: string; employeeId: string; userId: string | null }>;
          ...
          await this.prisma.$queryRawUnsafe(
            `INSERT INTO attendance.attendance_alerts
               (id, "employeeId", date, "alertType", "attendanceRecordId", severity,
                message, "messageAr", status, "isAutoGenerated", "createdAt", "updatedAt")
             VALUES
               (gen_random_uuid(), $1, $2::date, 'MISSING_CLOCK_IN', $3, 'MEDIUM', $4, $5,
                'OPEN', true, NOW(), NOW())`,
            r.employeeId, dateStr, r.recordId,
            `Employee did not clock in on ${dateStr}`,
            `الموظف لم يسجل دخولاً يوم ${dateStr}`,
          );
          ...
          if (r.userId) {
            await this.prisma.$queryRawUnsafe(
              `INSERT INTO users.notifications
               ...
               VALUES
                 (gen_random_uuid(), $1, 'ATTENDANCE_ALERT',
                  'تنبيه: نسيت تسجيل الدخول', 'Reminder: Missing Clock-In',
                  $2, $3, false, $4::jsonb, NOW())`,
              r.userId,
              `لم يتم تسجيل دخولك يوم ${dateStr} — يمكنك تقديم تبرير إذا كان هناك سبب.`,
              `You did not clock in on ${dateStr}. Please submit a justification if needed.`,
              JSON.stringify({ alertType: 'MISSING_CLOCK_IN', attendanceRecordId: r.recordId, date: dateStr }),
            );
          }
        } catch (err) {
          this.logger.error(`createMissingClockInAlerts failed for ${r.employeeId} on ${dateStr}: ${(err as any)?.message}`);
        }
      }

      if (records.length > 0) {
        this.logger.log(`createMissingClockInAlerts: ${records.length} MISSING_CLOCK_IN alert(s) created for ${dateStr}`);
      }
    } catch (err) {
      this.logger.error(`createMissingClockInAlerts failed for ${dateStr}: ${(err as any)?.message}`);
    }
  }
  ...
  private async createMissingClockOutAlert(employeeId: string, dateStr: string, salaryLinked = true, skipAlert = false) {
    ...
    try {
      ...
      const recRows = (await this.prisma.$queryRawUnsafe(
        `SELECT id FROM attendance.attendance_records WHERE "employeeId" = $1 AND date = $2::date LIMIT 1`,
        employeeId, dateStr,
      )) as Array<{ id: string }>;
      const attendanceRecordId = recRows[0]?.id ?? null;

      await this.prisma.$queryRawUnsafe(
        `INSERT INTO attendance.attendance_alerts
           (id, "employeeId", date, "alertType", "attendanceRecordId", severity, message, "messageAr",
            status, "isAutoGenerated", "createdAt", "updatedAt")
         VALUES
           (gen_random_uuid(), $1, $2::date, 'MISSING_CLOCK_OUT', $3, 'MEDIUM', $4, $5,
            'OPEN', true, NOW(), NOW())`,
        employeeId, dateStr, attendanceRecordId,
      ...
      if (empRows[0]?.userId) {
        await this.prisma.$queryRawUnsafe(
          ...
          JSON.stringify({ alertType: 'MISSING_CLOCK_OUT', attendanceRecordId, date: dateStr }),
        );
      }
