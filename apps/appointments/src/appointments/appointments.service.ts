...
export class AppointmentsService implements OnModuleInit {
  ...
  private async buildStatisticsRows(query: StatisticsQueryDto) {
    ...
    // المعالج المسؤول فعلياً عن الحالة: therapistIds هو الحقل الصحيح (تأكدنا بالبيانات الفعلية
    // إنو practitionerId=createdBy دائماً، أي موظف الاستقبال اللي سجّل الموعد، مش المعالج).
    const responsibleIds = (a: (typeof raw)[number]): string[] =>
      a.therapistIds?.length ? a.therapistIds : (a.physiotherapistId ? [a.physiotherapistId] : (a.practitionerId ? [a.practitionerId] : []));
    const practitionerIds = [...new Set(raw.flatMap(responsibleIds))];
    ...
    const rows = raw.map((a, idx) => ({
      ...
      practitionerName: responsibleIds(a).map(id => employeeMap.get(id)).filter(Boolean).join('، '),
